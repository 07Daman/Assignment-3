// Function to redirect to special offer page
function redirectToSpecialOffer() {
  window.location.href = "special_offer.html"; // Redirecting to special offer page
}

// Adding event listener to special offer button
document.addEventListener("DOMContentLoaded", function () {
  const specialOfferBtn = document.getElementById("specialOfferBtn");

  specialOfferBtn.addEventListener("click", redirectToSpecialOffer); // Redirecting to special offer page on button click
});
...
// Function to toggle navigation menu display
function toggleNavMenu() {
  var nav = document.getElementById("navMenu");
  if (nav.style.display === "none") {
    nav.style.display = "block"; // Displaying navigation menu if it's hidden
  } else {
    nav.style.display = "none"; // Hiding navigation menu if it's visible
  }
}
